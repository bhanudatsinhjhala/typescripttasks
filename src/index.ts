...
function demo(num:number, str:string): Demo {
    return { num, str};
}
...
function palindromeChecker(value:string):boolean{
    const reverString:string = value.split("").reverse().join("");
    if(reverString == value) return true;
    return false;
}
...
function convertUpperCase(arrString:string[]):string[] {
    arrString = arrString.map((string: string) => string.toUpperCase());
    return arrString;
  }
...
function evenNumbers(arrNum:number[]):number[]{
...
function betweenMarkers(sentence:string, initial:string, final:string): string{
